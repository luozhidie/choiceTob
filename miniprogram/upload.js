...
wp(distPath, 'home',
  ...
  '<view style="background:#2d1b2e;padding-top:50px;padding-bottom:30px;padding:20px">' +
  '<text style="font-size:24px;font-weight:bold;color:#fff">骆芷蝶供应链</text>' +
  '<text style="font-size:13px;color:#c4b5a8;margin-top:6px;display:block">服装门店一站式赋能平台</text>' +
  '<view bindtap="goBuyer" style="display:flex;align-items:center;background:rgba(255,255,255,0.12);border-radius:25px;padding:11px 16px;margin-top:24">' +
  ...
  '<view style="padding:8px"><text style="font-size:12px;color:#333">{{item.name}}</text>' +
  '<text style="font-size:14px;color:#e89a5c;font-weight:bold;display:block;margin-top:4px">¥{{item.pt}}</text></view></view></view></block>' +
  '<block wx:else><view style="padding:40px;text-align:center"><text style="font-size:30px">📦</text><text style="color:#999;font-size:13px;display:block;margin-top:10px">加载中</text></view></block></view>' +
  '<view style="padding:40px;text-align:center;padding-bottom:80px"><text style="font-size:18px;font-weight:bold;color:#2d1b2e">爆款选品·拿货精选</text>' +
  '<text style="font-size:12px;color:#888;margin-top:6px;display:block">骆芷蝶智选·专业推荐</text></view></view>',
  'Page({\n' +
  'data: {\n' +
  '  tags: ["全部","服装","护肤","彩妆"],\n' +
  '  products: []\n' +
  ...
  '      l.forEach(function(p) { p.pt = Number(p.price || 0).toFixed(2); });\n' +
...
wp(distPath, 'buyer',
  ...
  '<input placeholder="搜索商品..." style="background:#fff;border-radius:25px;padding:12px 16px;margin-top:16px;border:1px solid #ddd;font-size:14px"/>' +
  '</view>',
...
// CART
wp(distPath, 'cart',
  '<view style="background:#f8f7f4;height:100vh;display:flex;align-items:center;justify-content:center">' +
  '<text style="font-size:40px">🛒</text>' +
  '<text style="color:#999;font-size:15px;display:block;margin-top:16px">购物车是空的</text></view>',
  'Page({})'
);
...
wp(distPath, 'my',
  ...
  '<view style="width:60px;height:60px;border-radius:30px;background:#2d1b2e;display:flex;justify-content:center;align-items:center">' +
  '<text style="font-size:24px;color:#fff">骆</text></view>' +
  '<view style="margin-left:16px"><text style="font-size:17px;font-weight:bold;color:#333">骆芷蝶智选</text></view></view>' +
  '<view style="margin-top:24px;background:#fff;border-radius:16px"><text style="padding:18px 20px;border-bottom:1px solid #f5f5f5;display:block">我的订单</text></view></view>',
  'Page({})'
);

// PLACEHOLDERS
var ph = ['courses','daily-looks','shop','login'];
ph.forEach(function(pg) {
  wp(distPath, pg,
    '<view style="background:#f8f7f4;height:100vh;display:flex;align-items:center;justify-content:center">' +
    '<text style="font-size:40px">🚧</text>' +
    '<text style="color:#999;font-size:15px;display:block;margin-top:12px">' + pg.toUpperCase() + '</text></view>',
    'Page({})'
  );
});
