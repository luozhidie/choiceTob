...
export default function AdminSalesPlanPage() {
  ...
  // 用 admin API（service_role 绕过 RLS）加载店铺和已保存方案
  useEffect(() => {
    (async () => {
      try {
        ...
        if (json.plans) setSavedPlans(json.plans);
      } catch {}
    })();
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok || json.error) { alert("保存失败: " + (json.error || "未知错误")); return; }
      alert("方案已保存！");
      const list = await fetch("/api/admin/sales", { credentials: "include" });
      const lj = await list.json();
      if (lj.plans) setSavedPlans(lj.plans);
      setActiveTab("saved");
