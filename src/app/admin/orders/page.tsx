...
import { CheckCircle, Clock, XCircle, Truck, Eye, ShoppingBag, Trash2, Search } from "lucide-react";
...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string }> = {
  ...
  refund_pending: { label: "退货申请", color: "text-orange-700", bg: "bg-orange-50 border-orange-200" },
  refunded: { label: "已退款", color: "text-gray-700", bg: "bg-gray-50 border-gray-200" },
};
...
export default function OrdersPage() {
  ...
  const updateStatus = async (orderId: string, status: string, extra?: Record<string, unknown>) => {
    try {
      ...
      if (!error) {
        const updated = { ...orders.find(o => o.id === orderId), status, ...extra } as BuyerOrder;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setSearchTerm(""); fetchOrders(); }} className="px-4 py-2 bg-accent/10 text-accent rounded-lg hover:bg-accent/20 transition-colors text-sm font-medium">
          🔄 刷新
        </button>
      </div>
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-accent/20"
              placeholder="搜索：订单号、买家姓名、手机号、备注..."
            />
          </div>
          <button onClick={fetchOrders} className="px-6 py-2.5 bg-accent text-white rounded-lg text-sm font-medium hover:bg-accent/90">
            搜索
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 lg:grid-cols-8 gap-4">
        {[
          { label: "全部", count: orders.length, color: "bg-gray-50 text-gray-700" },
          ...
          { label: "已发货", count: orders.filter(o => o.status === "shipped").length, color: "bg-blue-50 text-blue-700" },
          { label: "已完成", count: orders.filter(o => o.status === "completed").length, color: "bg-emerald-50 text-emerald-700" },
          { label: "已取消", count: orders.filter(o => o.status === "cancelled").length, color: "bg-red-50 text-red-700" },
          { label: "退货申请", count: orders.filter(o => o.status === "refund_pending").length, color: "bg-orange-50 text-orange-700" },
          { label: "已退款", count: orders.filter(o => o.status === "refunded").length, color: "bg-gray-50 text-gray-700" },
        ].map((stat, i) => (
          <div key={i} className={`${stat.color} rounded-xl p-4 cursor-pointer hover:shadow-md transition-shadow`} onClick={() => {
            if (i < TABS.length) setActiveTab(TABS[i].key);
          }}>
        ...
        {TABS.map(tab => (
          <button key={tab.key}
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
              activeTab === tab.key ? "bg-accent text-white shadow-md" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
            }`}
          >
            {tab.label}
            {tab.key !== "all" && (
              <span className="ml-1.5 px-1.5 py-0.5 rounded-md text-[10px] bg-white/30">
                {orders.filter(o => tab.key === "refund_pending" ? o.status === "refund_pending" : o.status === tab.key).length}
              </span>
            )}
          </button>
        ))}
                ...
                return (
                  <tr key={order.id} className="hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium border ${st.bg} ${st.color}`}>
                        {st.label}
                      </span>
                    </td>
            ...
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }}
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="bg-accent/5 rounded-xl p-4 space-y-2">
                  ...
                  {selectedOrder.discount_rate && selectedOrder.discount_rate > 0 && (
                    <div className="flex justify-between text-sm text-green-600">
                      <span>会员折扣 ({((selectedOrder.discount_rate || 0) * 100).toFixed(0)}%)</span>
                ...
                {["shipped", "completed"].includes(selectedOrder.status) && (
                  <div className="space-y-2 text-sm bg-blue-50 rounded-xl p-4">
                    <div className="font-medium text-primary mb-1">🚚 物流信息</div>
                    {selectedOrder.tracking_company && (
                      <div className="flex justify-between"><span className="text-gray-500">快递公司</span><span>{selectedOrder.tracking_company}</span></div>
                    )}
                    {selectedOrder.tracking_number && (
                      <div className="flex justify-between"><span className="text-gray-500">运单号</span><span className="font-mono text-xs">{selectedOrder.tracking_number}</span></div>
                    )}
                    {selectedOrder.shipped_at && (
                      <div className="flex justify-between"><span className="text-gray-500">发货时间</span><span className="text-xs">{formatDate(selectedOrder.shipped_at)}</span></div>
                    )}
                  </div>
                )}
                ...
                {selectedOrder.status === "refunded" && (
                  <div className="space-y-2 text-sm bg-orange-50 rounded-xl p-4">
                    <div className="font-medium text-orange-800 mb-1">💰 退款信息</div>
                    {selectedOrder.refund_reason && (
                      <div className="flex justify-between"><span className="text-orange-600">退款原因</span><span>{selectedOrder.refund_reason}</span></div>
                    )}
                    {selectedOrder.refunded_at && (
                      <div className="flex justify-between"><span className="text-orange-600">退款时间</span><span className="text-xs">{formatDate(selectedOrder.refunded_at)}</span></div>
                    )}
                  </div>
                )}
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between"><span className="text-gray-500">状态</span>
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${STATUS_MAP[selectedOrder.status]?.bg} ${STATUS_MAP[selectedOrder.status]?.color}`}>{STATUS_MAP[selectedOrder.status]?.label || selectedOrder.status}</span></div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-4 border-t border-gray-100">
                {selectedOrder.status === "paid" && (
                  <button onClick={() => {
                    const company = prompt("请输入快递公司（如：顺丰、中通）：");
                    if (!company) return;
                    const trackingNo = prompt("请输入运单号：");
                    ...
                    className="py-2.5 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 flex items-center justify-center gap-1.5"
                  >
                    <Truck className="w-4 h-4" /> 标记发货
                  </button>
                )}
                {selectedOrder.status === "shipped" && (
                  <button onClick={() => updateStatus(selectedOrder.id, "completed", { completed_at: new Date().toISOString() })}
                    className="py-2.5 bg-green-600 text-white text-sm font-medium rounded-xl hover:bg-green-700 flex items-center justify-center gap-1.5"
                  >
                ...
                {selectedOrder.status === "refund_pending" && (
                  <>
                    <button onClick={() => {
                      ...
                      updateStatus(selectedOrder.id, "refunded", {
                        refund_reason: reason || "",
                        refunded_at: new Date().toISOString(),
                      });
                    }}
                      className="py-2.5 bg-orange-500 text-white text-sm font-medium rounded-xl hover:bg-orange-600 flex items-center justify-center gap-1.5"
                    >
                      💰 同意退款
                    </button>
                    <button onClick={() => updateStatus(selectedOrder.id, "shipped")}
                      className="py-2.5 bg-gray-400 text-white text-sm font-medium rounded-xl hover:bg-gray-500"
                    >
                      拒绝退款
                    </button>
                  </>
                ...
                {(selectedOrder.status === "pending" || selectedOrder.status === "paid") && (
                  <button onClick={() => updateStatus(selectedOrder.id, "cancelled")}
                    className="py-2.5 bg-red-500 text-white text-sm font-medium rounded-xl hover:bg-red-600 flex items-center justify-center gap-1.5"
                  >
                    <XCircle className="w-4 h-4" /> 取消订单
                  </button>
                )}
                <button onClick={() => setSelectedOrder(null)}
                  className="py-2.5 bg-gray-100 text-gray-700 text-sm font-medium rounded-xl hover:bg-gray-200"
                >
                  关闭
                </button>
              </div>
