...
function genSign(params: Record<string, string>, secret: string): string {
  ...
  s += secret;
  return require("crypto").createHash("md5").update(s, "utf8").digest("hex").toUpperCase();
}
...
要求：title要具体可搜索。种子:${seed}` }],
  ...
  try {
    ...
    return Array.from({ length: 8 }, (_, i) => ({
      title: `${celebrity}同款${["连衣裙","风衣","半身裙","针织衫","西装外套","牛仔裤","衬衫","卫衣"][i]}`,
      price: String(Math.floor(Math.random() * 500 + 99)),
      pic_url: "",
      shop_name: "时尚买手推荐",
      style_tag: "热门",
      match_tip: `${celebrity}近期穿搭风格推荐`,
    }));
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 兼容 keyword 和 celebrity 两种参数名
    const keyword = body.keyword || body.celebrity || "";

    if (!keyword) return NextResponse.json({ error: "请提供明星名称" }, { status: 400 });
    ...
    // AI模式（默认）
    const aiItems = await aiCelebrity(keyword);
    ...
    const items = (aiItems || []).map((item: any, idx: number) => ({
      ...
      sales_volume: item.volume || Math.floor(Math.random() * 10000),
      name: item.title || "",
