...
async function fetchMarkets(supabase: ReturnType<typeof createClient>, id?: string | null) {
  if (id) {
...
export async function GET(request: NextRequest) {
  ...
  if (process.env.SUPABASE_SERVICE_ROLE_KEY) {
    try {
      ...
      const { data, error } = await fetchMarkets(supabase, id);
