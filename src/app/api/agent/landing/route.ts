...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: agent } = await supabase
      .from("profiles")
      .select("id, full_name, agent_store_name, nickname, membership_type, deposit_amount, is_admin, store_owner_certified, role, is_tryon_agent")
      .eq("invite_code", ref)
      .maybeSingle();
    const isDepositAgent =
      !!agent && agent.membership_type === "deposit_discount" && Number(agent.deposit_amount || 0) > 0;
    const isTryonAgent = !!agent && agent.is_tryon_agent === true;
    const isPrivileged =
      !!agent && (agent.is_admin === true || agent.store_owner_certified === true || agent.role === "admin");
    const isValid = isDepositAgent || isPrivileged || isTryonAgent;
    if (!isValid) {
    ...
    // 对客展示：优先用店铺名，其次昵称，最后姓名兜底
    const agentStoreName = agent.agent_store_name || "";
    const agentName = agent.agent_store_name || agent.nickname || agent.full_name || "精选推荐";
