...
/* ==================== 预售模块（倒计时 + 商品） ==================== */
function PreSaleBlock({ content, block, bg }: { content: any; block: any; bg: string }) {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="rounded-2xl overflow-hidden shadow-sm" style={{ background: bg === "#ffffff" ? "linear-gradient(135deg,#f0e6ff 0%,#fff5ff 100%)" : bg }}>
        {content.bannerImage && (
        ...
        <div className="p-5 md:p-6">
          <div className="flex items-center justify-between mb-3">
            ...
            {(content.deposit > 0 || content.discount < 1) && (
              <div className="flex gap-2 flex-wrap justify-end">
                ...
                {content.discount < 1 && <span className="px-2 py-1 bg-pink-100 text-pink-700 rounded-full text-xs font-bold">{Math.round(content.discount * 10)}折</span>}
              </div>
            )}
          </div>
          {!ended ? (
            <div className="flex items-center gap-2 mb-4 p-3 bg-white/60 rounded-xl">
              <span className="text-xs font-bold text-purple-700 whitespace-nowrap shrink-0">⏰ 距截止</span>
              {[
                ...
                <div key={label} className="flex flex-col items-center shrink-0">
                  <div className={`w-10 h-10 rounded-lg ${val !== 0 ? "bg-gradient-to-br from-purple-500 to-pink-500 text-white" : "bg-gray-200 text-gray-400"} flex items-center justify-center text-base font-mono font-bold`}>{val}</div>
              ...
              {preProducts.map((p: any) => (
                <a key={p.id} href={`/shop/${p.id}`} className="group block">
                  <div className="rounded-xl overflow-hidden bg-white/50 mb-1.5 aspect-[3/4] relative">
                    ...
                    {content.discount < 1 && <div className="absolute top-1.5 left-1.5 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-[8px] px-1.5 py-0.5 rounded-full font-medium">预售</div>}
                  </div>
                  <h4 className="font-medium text-[11px] line-clamp-1 group-hover:text-purple-500 transition-colors">{p.name || p.title}</h4>
                  <p className="text-accent font-bold text-xs">{fmtPrice(p.price)}</p>
                  {content.discount < 1 && <p className="text-gray-400 text-[9px] line-through">{fmtPrice(Math.round(p.price / (content.discount || 1)))}</p>}
                </a>
              ))}
            </div>
          ) : (
          ...
          <a href="/buyer" className="block w-full mt-4 py-3 text-center bg-gradient-to-r from-purple-500 to-pink-500 text-white font-bold rounded-xl hover:shadow-lg transition-all text-sm">立即预订 →</a>
        </div>
      </div>
    </div>
  );
          ...
          {buyerBlocks.map((block: any) => {
            ...
            return (
              ...
              >
                ...
                {/* pre_sale 预售模块 */}
                {block.type === "pre_sale" && (
                  <PreSaleBlock content={content} block={block} bg={bg} />
                )}
            ...
            {memberPromptType === "view_price" ? (
              ...
              <>
                ...
                <div className="space-y-2 mb-4">
                  ...
                  <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-3 border-2 border-purple-300 cursor-pointer"
                       onClick={() => user ? router.push('/vip') : router.push('/login?redirect=/vip')}>
                    <div className="flex items-center justify-between">
                      <div>
