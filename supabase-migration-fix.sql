-- ============================================================
-- 迁移 1: hot_picks_memberships 表（爆款样衣独立会员 ¥998/月）
-- ============================================================
...
DROP POLICY IF EXISTS "Users can view own hot picks membership" ON hot_picks_memberships;
CREATE POLICY "Users can view own hot picks membership"
  ON hot_picks_memberships FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can manage hot picks memberships" ON hot_picks_memberships;
...
DROP POLICY IF EXISTS "Users can view own diagnoses" ON style_diagnoses;
CREATE POLICY "Users can view own diagnoses"
  ON style_diagnoses FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own diagnoses" ON style_diagnoses;
CREATE POLICY "Users can insert own diagnoses"
  ON style_diagnoses FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can manage all diagnoses" ON style_diagnoses;
