...
Page({
  ...
  startImport: function () {
    ...
    // 检测微信小程序 / 非网页链接：一键导入不支持，提示用「相册抓取」
    var mpHit = urls.filter(function (u) {
      return /#小程序[:/]|wxapurl:|weixin(\.qq)?\.com|wxde[0-9a-f]+/i.test(u);
    });
    if (mpHit.length > 0) {
    ...
    if (unsupported.length > 0) {
      wx.showModal({
        title: '该链接不支持自动导入',
        content: '淘宝/天猫/1688/京东/抖音/拼多多/小红书等页面为动态加载，无法通过链接自动抓取。请在商品页长按图片 → 保存图片到相册 → 使用「智能建商品」上传保存的图片，AI 自动识别标题、价格、尺码、材质。',
        confirmText: '去智能建商品',
        cancelText: '取消',
        success: function (r) {
