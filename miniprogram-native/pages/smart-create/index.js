...
Page({
  data: {
    ...
    stylePopupOpen: false,   // 风格多选弹窗
    stylePopupOptions: [],   // [{label, selected}]
    styleSelected: [],       // string[] 当前已选风格
  ...
  /* 风格选择（多选，逗号分隔写入 params.style，供两级联动筛选与商品检索） */
  openStylePopup: function () {
    var cur = (this.data.product && this.data.product.style) || "";
    var sel = cur.split(",").map(function (s) { return s.trim(); }).filter(Boolean);
    var opts = (this.data.styleOptions || []).map(function (label) {
      return { label: label, selected: sel.indexOf(label) >= 0 };
    });
    this.setData({ stylePopupOpen: true, stylePopupOptions: opts, styleSelected: sel });
  },
  toggleStylePopupItem: function (e) {
    var i = e.currentTarget.dataset.i;
    var opts = this.data.stylePopupOptions.slice();
    var label = opts[i].label;
    opts[i].selected = !opts[i].selected;
    var sel = opts.filter(function (o) { return o.selected; }).map(function (o) { return o.label; });
  ...
  confirmStylePopup: function () {
    ...
    this.setData({ product: p, stylePopupOpen: false });
  },
