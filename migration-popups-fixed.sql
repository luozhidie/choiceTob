...
drop policy if exists "public_read_published_popups" on public.popups;
create policy "public_read_published_popups"
  on public.popups
  for select
  using (
    is_published = true
...
drop policy if exists "admin_full_access_popups" on public.popups;
create policy "admin_full_access_popups"
  on public.popups
  for all
  using (true)
  with check (true);
